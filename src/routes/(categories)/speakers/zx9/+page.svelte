<script lang="ts">
	...
	import { markone, xx59, zx7, zx9 } from '$lib/data/productdetails.json';
...
<main class="mx-[1.51rem] md:mx-[1em] mb-24">
	...
	<section
		class="p-3 md:p-4 lg:p-5 flex flex-col md:flex-row gap-2 md:gap-8 lg:gap-10 text-start md:items-center justify-center"
	>
		<enhanced:img
			src="/src/lib/assets/product-zx7-speaker/desktop/image-category-page-preview.jpg"
			class="rounded-md"
			alt="yx1-earphones"
		/>
		<div class="flex flex-col gap-4 md:gap-2 lg:self-center">
			<p class="tracking-wide uppercase text-orange font-sans">New Product</p>
			<h1 class="uppercase text-3xl lg:text-5xl font-bold">{zx9[0].name}</h1>
			<p class="text-zinc-700">
				{zx9[0].description}
			</p>
			<p class="font-bold lg:py-4">$ {zx9[0].price}</p>
				...
				<button
					...
					onclick={() => {
						cartProducts.push({
							...
							quantity: value,
							price: zx9[0].price,
							thumbmail: '/src/lib/assets/cart/image-zx9-speaker.jpg'
						});
					}}>Add to Cart</button
				>
			</div>
	...
	>
		...
		<div class="flex flex-col gap-3 md:gap-2">
			<div class="flex gap-x-4">
				<p class="text-orange font-bold">{zx9[0].includes[0].quantity}x</p>
				<p>{zx9[0].includes[0].item}</p>
			</div>
			<div class="flex gap-x-4">
				<p class="text-orange font-bold">{zx9[0].includes[1].quantity}x</p>
				<p>{zx9[0].includes[1].item}</p>
			</div>
			<div class="flex gap-x-4">
				<p class="text-orange font-bold">{zx9[0].includes[2].quantity}x</p>
				<p>{zx9[0].includes[2].item}</p>
			</div>
			<div class="flex gap-x-4">
				<p class="text-orange font-bold">{zx9[0].includes[3].quantity}x</p>
				<p>{zx9[0].includes[3].item}</p>
			</div>
			<div class="flex gap-x-4">
				<p class="text-orange font-bold">{zx9[0].includes[4].quantity}x</p>
				<p>{zx9[0].includes[4].item}</p>
			</div>
		</div>
	</section>
	<!-- Gallery -->
	<section
		class="flex flex-col gap-2 md:gap-[1.3rem] lg:gap-[1.4rem] md:flex-row pb-4 md:pb-8 lg:pb-12 items-center xl:gap-[2rem]"
	>
		<div class="flex flex-col gap-2 md:gap-[1.4em] lg:gap-[1.85rem] xl:gap-[2rem]">
			<enhanced:img
				src="/src/lib/assets/product-zx7-speaker/desktop/image-gallery-1.jpg"
				class="rounded-md"
				alt="zx7-speakers"
			/>
			<enhanced:img
				src="/src/lib/assets/product-zx7-speaker/desktop/image-gallery-2.jpg"
				class="rounded-md"
				alt="zx7-speakers"
			/>
		</div>
		<enhanced:img
			src="/src/lib/assets/product-zx7-speaker/desktop/image-gallery-3.jpg"
			class="rounded-md"
			alt="zx7-speakers"
		/>
	</section>
	...
	<section class="pb-12 md:pb-16 lg:pb-24">
		...
		<div class="wrapper flex flex-col gap-5 md:gap-2 lg:gap-5 md:flex-row text-center">
			<div class="flex flex-col gap-4">
				<enhanced:img
					src="/src/lib/assets/product-zx7-speaker/mobile/image-product.jpg"
					class="rounded-md"
					alt="zx7-speakers"
				/>
				<h3 class="font-bold text-2xl uppercase">{zx7[0].short}</h3>
				<div class="block">
					<a href="/speakers/zx7">
						<SeeButton />
					</a>
